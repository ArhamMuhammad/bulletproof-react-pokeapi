...
describe('http service', () => {
  ...
  describe('handleResponse', () => {
    it('should throw an error if response is not ok', async () => {
      ...
      await expect(http.get('http://example.com')).rejects.toThrow(
        'Server error'
      );
    });
  ...
  describe('http.post', () => {
    it('should make a POST request and return data', async () => {
      ...
      const result = await http.post('http://example.com', { key: 'value' });
      expect(result).toEqual(mockData);
      expect(global.fetch).toHaveBeenCalledWith('http://example.com', {
        method: 'POST',
  ...
  describe('http.put', () => {
    it('should make a PUT request and return data', async () => {
      ...
      const result = await http.put('http://example.com', { key: 'value' });
      expect(result).toEqual(mockData);
      expect(global.fetch).toHaveBeenCalledWith('http://example.com', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  describe('http.delete', () => {
    it('should make a DELETE request and return data', async () => {
      ...
      const result = await http.delete('http://example.com');
      expect(result).toEqual(mockData);
      expect(global.fetch).toHaveBeenCalledWith('http://example.com', {
        method: 'DELETE'
      });
